...
import com.fourdays.core.url.adapter.out.persistence.ProtocolRepositoryStub;
import com.fourdays.core.url.adapter.out.persistence.UrlRepositoryStub;
...
class UrlServiceImplTest {
    ...
    void shortenUrlTest_urlKeyCheck() {
        UrlService urlService = new UrlServiceImpl(() -> "urlKey", new UrlRepositoryStub(), new ProtocolServiceImpl(new ProtocolRepositoryStub()));
    ...
    void findOriginalUrlByKeyTest_exists() {
        UrlRepositoryStub urlRepository = new UrlRepositoryStub();
        Protocol https = new Protocol(1, "HTTPS");
        urlRepository.save(new URL("urlKey", https, "https://four.days/"));
        UrlServiceImpl urlService = new UrlServiceImpl(new Base62EncoderUsingNanoTime(), urlRepository, new ProtocolServiceImpl(new ProtocolRepositoryStub()));
